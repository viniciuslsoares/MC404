...
int main() {
    ...
    read(0, entrada, 32);
    InstData *inst;
    get_inst_data(entrada, inst);
    int output_dec = 0;

    pack(inst -> opcode, 0, 6, &output_dec);
    switch (inst -> type)
    {
    case R:
        pack(inst -> rd, 7, 11, &output_dec);
        pack(inst -> funct3, 12, 14, &output_dec);
        pack(inst -> rs1, 15, 19, &output_dec);
        pack(inst -> rs2, 20, 24, &output_dec);
        pack(inst -> funct7, 25, 31, &output_dec);
        break;
    case I:
        pack(inst -> rd, 7, 11, &output_dec);
        pack(inst -> funct3, 12, 14, &output_dec);
        pack(inst -> rs1, 15, 19, &output_dec);
        pack(inst -> imm, 20, 31, &output_dec);
        break;
    case S:
        pack(inst -> imm, 7, 11, &output_dec);
        pack(inst -> funct3, 12, 14, &output_dec);
        pack(inst -> rs1, 15, 19, &output_dec);
        pack(inst -> rs2, 20, 24, &output_dec);
        pack((inst -> imm >> 5), 25, 31, &output_dec);
        break;
    case B:
        pack((inst -> imm) >> 11, 7, 7, &output_dec);
        pack((inst -> imm >> 1), 8, 11, &output_dec);
        pack(inst -> funct3, 12, 14, &output_dec);
        pack(inst -> rs1, 15, 19, &output_dec);
        pack(inst-> rs2, 20, 24, &output_dec);
        pack((inst -> imm >> 5), 25, 30, &output_dec);
        pack((inst -> imm >> 12), 31, 31, &output_dec);
        break;
    case U:
        pack(inst -> rd, 7, 11, &output_dec);
        pack(inst -> imm, 12, 31, &output_dec);
        break;
    case J:
        pack(inst -> rd, 7, 11, &output_dec);
        pack((inst -> imm >> 12), 12, 19, &output_dec);
        pack((inst -> imm >> 11), 20, 20, &output_dec);
        pack((inst -> imm >> 1), 21, 30, &output_dec);
        pack((inst -> imm >> 20), 31, 31, &output_dec);
        break;
